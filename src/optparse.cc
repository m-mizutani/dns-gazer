...
namespace optparse {
  ...
  class str_wrap {
  public:
    explicit str_wrap(const std::string& l, const std::string& r) :
      lwrap(l), rwrap(r) {}
  ...
  static std::string str_join_trans(const std::string& sep, InputIterator begin,
  ...
  Option& OptionParser::add_option(const std::string& opt1,
  ...
  OptionParser& OptionParser::add_option_group(const OptionGroup& group) {
    for (std::list<Option>::const_iterator oit = group._opts.begin();
         oit != group._opts.end(); ++oit) {
      const Option& option = *oit;
      for (std::set<std::string>::const_iterator it
             = option._short_opts.begin();
  ...
  void OptionParser::handle_short_opt(const std::string& opt,
  ...
  const Option& OptionParser::lookup_long_opt(const std::string& opt) const {
    ...
    for (optMap::const_iterator it = _optmap_l.begin();
         it != _optmap_l.end(); ++it) {
    ...
    if (matching.size() > 1) {
      ...
      error(_("ambiguous option") + std::string(": --") +
            opt + " (" + x + "?)");
    }
    if (matching.size() == 0)
      error(_("no such option") + std::string(": --") + opt);
  ...
  void OptionParser::handle_long_opt(const std::string& optstr) {
    ...
    if (option._nargs == 1 && delim == std::string::npos) {
      if (!_remaining.empty()) {
        value = _remaining.front();
        _remaining.pop_front();
      }
  ...
  Values& OptionParser::parse_args(const int64_t argc,
  ...
  Values& OptionParser::parse_args(const std::vector<std::string>& v) {
    ...
    if (add_version_option() && version() != "") {
      add_option("--version") .action("version")
    ...
    for (strMap::const_iterator it = _defaults.begin();
         it != _defaults.end(); ++it) {
      if (!_values.is_set(it->first))
        _values[it->first] = it->second;
    }

    for (std::list<Option>::const_iterator it = _opts.begin();
         it != _opts.end(); ++it) {
  ...
  void OptionParser::process_opt(const Option& o, const std::string& opt,
  ...
  std::string OptionParser::format_option_help(unsigned int indent) const {
    std::stringstream ss;
  ...
  std::string OptionParser::format_help() const {
    ...
    for (std::list<OptionGroup const*>::const_iterator it = _groups.begin();
         it != _groups.end(); ++it) {
  ...
  ////////// class Option { //////////
  std::string Option::check_type(const std::string& opt,
    ...
    if (type() == "int" || type() == "long") {
      int t;
      if (!(ss >> t))
        err << _("option") << " " << opt << ": " << _("invalid integer value")
            << ": '" << val << "'";
      ...
      if (!(ss >> t))
        err << _("option") << " " << opt << ": " << _("invalid complex value")
            << ": '" << val << "'";
    }
  ...
  std::string Option::format_option_help(unsigned int indent) const {
    ...
    if (!_long_opts.empty()) {
      ss << str_join_trans(", ", _long_opts.begin(), _long_opts.end(),
                           str_wrap("--", mvar_long));
    }
