...
#ifndef APPS_OPTPARSE_H_
#define APPS_OPTPARSE_H_
...
namespace optparse {
  ...
  public:
    ...
    Values& parse_args(int64_t argc, char const* const* argv);
  ...
  private:
    ...
    void process_opt(const Option& option, const std::string& opt,
  ...
  OptionGroup(const OptionParser& p, const std::string& t,
    ...
    OptionGroup& title(const std::string& t) {
      _title = t; return *this;
    }
    OptionGroup& group_description(const std::string& d) {
      _group_description = d; return *this;
    }
  ...
  private:
    std::string check_type(const std::string& opt,
  ...
  class Callback {
  public:
    virtual void operator() (const Option& option, const std::string& opt,
                             const std::string& val,
